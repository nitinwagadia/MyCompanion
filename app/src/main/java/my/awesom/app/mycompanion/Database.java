...
public class Database extends SQLiteOpenHelper {
    ...
    private static final String TABLE_LOCATION_INFO = "create table if not exists " + LOCATION_INFO_TABLE_NAME + " (" + EVENT_ID + " integer," + ADDRESS + " text , " + TRANSITION_TYPE + " integer, foreign key(" + EVENT_ID + ") references " + GENERAL_TABLE_NAME + " (" + EVENT_ID + "));";
    ...
    public static void getAllLocationEvents() {
        ...
        while (cursor.moveToNext()) {
            ...
            if (isPast == Constants.IS_PAST) {
                Constants.pastEvents.add(new MyEventDetails(message, null, address, event_id, isPast, event_type, transition_type));
            } else {
                Constants.scheduledEvents.add(new MyEventDetails(message, null, address, event_id, isPast, event_type, transition_type));
            }
        }
        cursor.close();
